...
1. Handoff `docs/handoffs/HANDOFF_<heute>_$BAU_TICKET.md` im Ticket-Worktree schreiben: Stand, nächste Schritte, offene Punkte. **Pflichtzeile `Umzug: server`**, **nie** die Zeile `Staffel: weiter` (sonst startet die lokale Staffel neu).
2. Eigene Arbeit mit Pathspec committen (`git status --short` vorher, nur eigene Dateien).
...
4. **Exit 0 → nichts mehr tun.** Die Session läuft schon auf dem Server; `bau.py` beendet diese lokale Session gleich selbst (kein Doppel-Lauf).
   Exit 3 = Weigerung (Meldung nennt die Abhilfe, beheben, erneut) · Exit 1 = Server-Start nicht bewiesen, die lokale Session arbeitet normal weiter.

Das Skript committet nur den Handoff (Betreff endet mit `(#<N>) [skip ci]`), pusht den Branch, startet auf dem Server `bash scripts/spawn_srv.sh <S> --tickets <N> --ohne-wache --umzug <branch>:<pfad>` und beendet die lokale Session erst, wenn tmux-Fenster `bau <N>` und `sessions <S>` den Lauf zeigen.
...
2. Ja → im Repo-Wurzelordner:
   `python3 "${TO_SPAWN_HOME:-$HOME/.claude/skills/to-spawn}"/to_spawn.py umzug-alle $TO_SPAWN_WACHE_SPEC`
3. Ergebniszeilen je Ticket melden (umgezogen / übersprungen / Stopp + Grund).

Ablauf streng nacheinander, nie zwei Sessions halb: wartende `bau.py` werden beendet und auf dem Server neu gestartet, laufende Sessions bekommen über ihren Stop-Hook die Anweisung aus Abschnitt 1 (Anfrage-Datei `.to-spawn/umzug-anfrage-<N>`). `VERWAIST` oder Zeitüberschreitung → Stopp, der Rest bleibt lokal. Zum Schluss startet der Wächter auf dem Server, diese Wächter-Session endet dann selbst.
