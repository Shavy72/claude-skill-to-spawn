...
- `/to-spawn-remote <S>` — Alias-Skill, `--ziel srv`: Bau-Server (tmux `spec-<S>`, Laptop darf aus). Ersetzt `/to-spawn srv <S>`. Ausgeführt wird `scripts/spawn_srv.sh <S> [--tickets a,b]` im Repo-Wurzelordner — unter Linux/macOS direkt per `bash`, auf einem Windows-Laptop per SSH-Sprung braucht der lokale Aufruf zusätzlich `pwsh`. **Auf dem Bau-Server selbst** (Linux) läuft `bash scripts/spawn_srv.sh <S> [--tickets a,b]` immer ohne `pwsh`.
...
- Logik liegt in `~/.claude/skills/to-spawn/skripte/` (`bau.py`, `wache.py`, `sessions_stand.py`, `spec_stand.py`, `spawn_srv.sh`, `aufpasser.py`), direkt startbar im Repo-Wurzelordner. Repo = `TO_SPAWN_REPO`, sonst Git-Wurzel des aktuellen Ordners — nie der Skill-Ordner. GitHub-Slug (`owner/name`) kommt aus dem origin; zeigt der nicht auf github.com, gilt nur die ausdrückliche Vorgabe `TO_SPAWN_GH_REPO`, sonst Abbruch Exit 2 „Kein GitHub-Repo erkannt“ (kein stiller Rückfall, #257 F5).
...
- Repo-Konfig `.to-spawn/config.json` (legt `bau.py`/`wache.py`/`to_spawn.py pruefen|spawn` beim ersten Start mit Vorgaben an, überschreibt nie): `terminal`, `ziel_default`, `ssh_ziel`, `runner`, `modelle` + `effort` je Rolle (`ticket`, `ticket_leicht`, `waechter`), `staffel`, `mail.ziel`, `regularien.checkpoint_label`, `staging_start`, `deploy_befehl`, `worktree_basis`, `hauptzweig`, `speicher`. Verdrahtet sind heute: `ziel_default`, `ssh_ziel`, `modelle.ticket` (`bau`, Vorrang `--model` > Konfig > `_default.json`), `modelle.waechter` (`wache`), `regularien`, `staffel`, `worktree_basis` (Basis der Ticket-Worktrees, #257), `hauptzweig` (Name des Hauptzweigs, leer = automatisch aus `gh.hauptzweig`: `origin/HEAD`, sonst der erste vorhandene Kandidat, #257), `speicher` (Speicher-Schutz vor jedem Start, siehe unten). `terminal` ist ein Objekt je Plattform (`{"win32": "wt", "linux": "tmux", "darwin": "tmux"}`), weil die eingecheckte Konfig zwischen Windows und Server geteilt wird; ein alter Text-Wert wird beim nächsten Setup umgewandelt. `terminal`, `modelle.ticket_leicht` und `effort` setzt das Setup (#208), gelesen werden sie aber erst von den Folge-Tickets der Spec #202 — eine Änderung dort wirkt noch nicht. Die übrigen Felder sind angelegt, aber ebenfalls noch nicht verdrahtet. `--dry-run` legt nichts an. `.gitignore`: `.to-spawn/*` + `!.to-spawn/config.json`. Checkpoint-Label (`regularien.checkpoint_label`, Vorgabe `checkpoint:human`): `to_spawn.py setup` legt es per `gh.label_sicherstellen` im GitHub-Repo an, falls es fehlt — ein vorhandenes Label (Farbe/Beschreibung) wird nie überschrieben. Vertrauens-Dialog von Claude Code („Do you trust the files in this folder?“): `to_spawn.vertrauen` bestätigt ihn automatisch in `~/.claude.json` für Worktree und Repo-Wurzel, bevor `claude` startet — keine Rückfrage in der unbeaufsichtigten Session.
- Der Staffel-Hook: Skill-eigene Kopie `skripte/hooks/staffel_stop.py` (greift immer), eine Repo-Kopie `scripts/hooks/staffel_stop.py` hat Vorrang, falls vorhanden.
- **Aufpasser** (#236, `skripte/aufpasser.py`, Cron alle 15 min auf dem Bau-Server, nur Linux/tmux): Hausmeister über Wächter und Bau-Sessions — je `spec-<S>` fehlende `bau <N>`-Fenster starten, Fenster geschlossener Tickets schließen, stille Fenster wecken. Sicherheitskette je Fenster: anstupsen → Worktree als Commit auf `sicherung/<N>` sichern + pushen (nie stash/reset, ohne `.env`/Zugangsdateien, Hauptrepo nie), Fenster per tmux-Respawn mit `bau.py --resume <Gesprächs-ID>` fortsetzen (Fenster + Sitzung bleiben, Nachweis binnen 120 s) → danach nur noch „braucht David“; ändert sich der Pane-Text nach einem Eingriff, gilt er als Erfolg (Stufe 0). Sicherheitskette gilt auch für das Wächter-Fenster (`wache.py --resume <Gesprächs-ID>`, ohne Sicherung — kein Worktree). Gesprächs-IDs überleben das Fenster: `bau.py`/Wächter schreiben `.to-spawn/sessions/<N>.json` bzw. `wache-<S>.json`; ein fehlendes Fenster oder ein Pane, in dem nur noch die Shell lebt, wird damit per `--resume` fortgesetzt (sofern das Transkript da ist), nie angestupst. Jeder Eingriff (gestartet, angestupst, fortgesetzt, geschlossen) bekommt seine eigene Issue-Zeile; nur Meldungen ohne Eingriff kommen einmal am Tag. Mehr als 3 Anstupser je Fenster in 24 h ⇒ „braucht David“. Direkt vor jedem Eingriff prüft die Deploy-Wache noch einmal; ein Start gilt erst, wenn das Fenster 5 s lang bleibt. Gesprächs-ID und `busy`/`idle` kommen aus `~/.claude/sessions/<pid>.json`; `--hang-min` nie unter 61 (wartende Loop-Sessions wachen spätestens alle 60 min auf). Ohne Sicherung wird nie ein Fenster geschlossen. Deploy/Gate läuft (auch Skripte aus `.to-spawn/config.json`, Prozesse > 6 h = Waise) → Lauf ohne Eingriff; Tickets mit `ready-for-human`/`needs-info`/`wontfix` werden nie gestartet, höchstens 2 Starts je Ticket in 6 h. `bau.py --resume` startet nur mit vorhandenem Transkript. Zustand `~/.local/state/to-spawn/aufpasser/`, Meldungen als Kommentar im Spec-Issue, `--trocken` zeigt nur. `bau.py` gibt jeder Session eine feste Gesprächs-ID (`--session-id`, Datei `session-id.txt`, Env `BAU_SESSION_ID`).
...
## Probesitz (`/to-spawn probesitz`, #214) — Abnahme des Setups

`python ~/.claude/skills/to-spawn/to_spawn.py probesitz` führt 7 echte Prüfungen selbst aus und merkt sich den Stand je Maschine + Repo; `setup --zeigen` und jedes Setup-Ende zeigen den Block „Probesitz (7 Punkte)“ mit dem, was fehlt. Punkte: (1) Login trägt · (2) Wegwerf-Session baut Mini-Commit + Push · (3) Playwright gegen Staging · (4) Sandbox sperrt außerhalb Worktree · (5) Bau-Log-Zeile mit Token · (6) künstlicher Handoff startet Folge-Session · (7) Mail „Probesitz grün“. Rot = Grund + „fehlt noch: …“.

- 2/5/6 laufen als EINE Wegwerf-Session: `gh issue create` (Titel „Probesitz <Datum>“), Worktree `wt-<N>`, `bau.py <N> --probesitz` (claude -p, Sonnet, Staffel-Max 2, fester Auftrag: Zweig `probesitz-<N>`, zwei Wegwerf-Commits, Handoff mit „Staffel: weiter“). Danach immer aufräumen: Ticket schließen, Fern-Zweig, Worktree und lokale Zweige löschen — auch bei Fehlern.
- `--punkt N` (mehrfach) prüft nur diese Punkte, `--zeigen` nur den Stand, `--modell` das Modell der Wegwerf-Session. Exit 0 = 7/7, 1 = mindestens ein ✗, 2 = Konfig/Repo unlesbar.
- Punkt 3 braucht `staging.url` in `.to-spawn/config.json` (oder `adresse=` in `staging.zugang_datei`) und Playwright im Python-Interpreter, der den Skill fährt. Punkt 7 braucht `mail.befehl`; die Art `probesitz_gruen` geht auch bei `nur_kritisch` raus.
...
## Speicher (#257 Paket B, seit 2026-09-21)

Obergrenze 6 Claude-Sessions je 16 GB RAM, Mindestmaß 2 GB frei, Staffel 20 s zwischen zwei Fenster-Starts — Konfig-Feld `speicher` (`min_frei_mib`, `max_sessions`, `staffel_s`) in `.to-spawn/config.json`. `bau <N>`, `wache <S>`, `spawn_srv.sh` und der Aufpasser fragen vor jedem Start `to_spawn.py speicher` (Exit 0 frei / 5 voll) und starten bei „voll“ nicht (Meldung „nicht gestartet: …“, nächster Aufpasser-Tick prüft neu). Das Nest legt Swap in RAM-Größe (mind. 8 GiB) und den tmux-Server als Dienst `tmux-bau.service` (OOMScoreAdjust=-900) an, damit ein OOM-Kill nie mehr alle Sessions mitnimmt.
...
- Wächter (#213): `wache <S>` startet mit `--remote-control "Wächter #<S>"` + `--fallback-model` und wechselt beim Nutzungs-Limit selbst auf `modelle.waechter_ausweich` (Aufsicht liest das Transkript, Bau-Log `waechter_modell`, Mail). Steht auch das Ausweich-Modell im Limit (#254): der Wächter liest die Reset-Uhrzeit aus derselben Limit-Zeile (`quotaLimits.resetsAt`, sonst „resets 5:40am (Europe/Berlin)“), pausiert bis dahin plus `TO_SPAWN_RESET_PUFFER_S` (Vorgabe 120 s) und fährt selbst per `--resume` weiter — Bau-Log `waechter_pause` + `waechter_weiter`, Mail `waechter_pause`. Ohne Uhrzeit, bei mehrdeutiger Uhrzeit (ohne am/pm), bei einem Reset über 6 h entfernt (`TO_SPAWN_RESET_MAX_S`) oder nach drei Pausen in Folge bleibt es bei Mail `session_tot`, Bau-Log `blockiert` und Stillstand. Windows: Zeitzonen-Namen braucht Python dort aus `pip install tzdata`, sonst gilt die Ortszeit des Rechners. Tick = `python scripts/capo.py <S> [--dry-run] [--kein-katalog] [--uebersicht]`: Stand je Ticket, nur neue Bau-Log-Zeilen, Regeln commit_ohne_nummer · beweis_fehlt · test_ersetzt · vps_ungleich_origin (öffnet wieder, je Schließ-Ereignis einmal) und session_verwaist (Kommentar + Mail, je Tag einmal); Mail nur kritisch (Gate rot, Session tot, Live-Beweis blockiert, Ausweich-Modell) + Spec fertig über `mail.befehl` (leer = keine Mail, nur INFO-Zeile, kein Fehler). Karenz nach dem Schließen: `waechter.karenz_minuten` (Vorgabe 15, `0` = keine); Ausgangsstand = beim ersten Tick schon geschlossene Tickets. Sessions melden `… eintrag --typ blockiert --grund "…"` bzw. `--typ entscheidung --frage … --wahl … --grund …`.
- Lernschleife (#286): jeder erkannte Stillstand wird ein **Vorfall** — Bau-Log-Zeile `vorfall` mit Klasse (`extern`/`infra`/`skill`/`prozess`/`mensch`), Symptom, Ursache, Lösung. capo schreibt sie bei jedem Verstoß, der Aufpasser bei `startet_nicht`/`start_fehlgeschlagen`/`fortsetzen_fehlgeschlagen`/`sicherung_fehlgeschlagen`/`stupser_erschoepft`/`braucht_david`/`rueckfrage`, eine Session selbst mit `… eintrag --typ vorfall --klasse … --symptom … --ursache … --loesung …`. Jeder Tick hängt neue Vorfälle als Tabellenzeile an den passenden Abschnitt von `docs/agents/FEHLERKATALOG_spawn.md` (Konfig `regularien.fehlerkatalog`) — je Regel genau einmal, Doppelte fallen raus; `--kein-katalog` schaltet das ab. Fehlt der Abschnitt oder ist die Tabelle kaputt, meldet der Tick `FEHLER: Katalog — …`; ein Repo ganz ohne Katalog bleibt grün.
